...
/**
 * Things mounted on a room wall hide along with it when that wall is cut
 * away. `wall` is the wall's outward normal.
 */
function OnWall({
  wall,
  children,
  ...
  return wall ? (
    <Cutaway normal={wall} full={children} stub={null} />
  ) : (
    <>{children}</>
  );
}
...
export function CoffeeStation({
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      <TiledBox
        size={[solid, H - 0.06, D]}
        at={[x0 + solid / 2, (H - 0.06) / 2, 0]}
      />
      <TiledBox size={[W, 0.06, D]} at={[0, H - 0.03, 0]} />
      <TiledBox
        size={[0.04, H - 0.06, D]}
        at={[W / 2 - 0.02, (H - 0.06) / 2, 0]}
      />
      {lines.map((w, i) => (
        <Box
          key={i}
          size={[w, lineH, 0.005]}
          at={[x0 + solid / 2, 0.74 - i * 0.13, D / 2 + 0.003]}
          m={pink}
          shadow={false}
        />
      ))}
      ...
      <group position={[x0 + solid + cubby / 2 - 0.02, 0, 0]}>
        <Box
          size={[cubby - 0.1, 0.62, D - 0.1]}
          at={[0, 0.31, 0]}
          m={flat("white", 3)}
        />
        <Box
          size={[cubby - 0.12, 0.012, 0.01]}
          at={[0, 0.5, D / 2 - 0.045]}
          m={flat("white", 1)}
          shadow={false}
        />
        <Box
          size={[(cubby - 0.12) / 2, 0.14, D - 0.15]}
          at={[-(cubby - 0.12) / 4, 0.7, 0]}
          m={flat("mustard", 2)}
        />
        <Box
          size={[(cubby - 0.12) / 2, 0.14, D - 0.15]}
      ...
      <group position={[0, H, 0]}>
        ...
        <mesh position={[0.15, 0.02, 0.05]} material={glow("glass", 2)}>
          <sphereGeometry
            args={[0.11, 10, 6, 0, Math.PI * 2, 0, Math.PI / 2]}
          />
        </mesh>
        <Cyl
          r={0.08}
...
export function FloorPlate({
  ...
  return (
    <Box
      size={[w, 0.02, d]}
      at={[position[0], position[1] + 0.01, position[2]]}
      m={flat(color)}
      shadow={false}
    />
  );
...
export function WallPanel({
  ...
  return (
    <OnWall wall={wall}>
      <group position={position} rotation={[0, rotation, 0]}>
        <mesh
          position={[0, h / 2, 0.012]}
          material={flat(color, shade)}
          receiveShadow
          userData={{ pixel: { dither: true } }}
        >
